...
def submit_complaint(conn, resident_id: int, data: dict):
    try:
        ...
        cursor.execute(
            "SELECT resident_id FROM resident WHERE resident_id = %s",
            (resident_id,)
        )
        ...
        return {
            ...
            "complaint_id": new_complaint[0] if new_complaint else None,
        }
        ...
        return [
            {
                ...
                "status": r[3],
                "created_at": str(r[4]) if r[4] else None,
                "resident_name": f"{r[5]} {r[6]}" if r[5] and r[6] else None,
...
def update_complaint_status(conn, complaint_id: int, status: str):
    try:
        ...
        cursor.execute(
            "SELECT complaint_id FROM complaint WHERE complaint_id = %s",
            (complaint_id,)
        )
